...
class AbstractCatalogUrlRewrites extends Command
{
    ...
    {
        $sql = "
SELECT `value`, min(row_id) as master_entity_id, COUNT(*) as c 
...
";
    ...
    public function dedupUrlAttribute($entityValue, $attributeId, $masterEntityId, $storeId, $additional = [])
    {
        ...
        $sql = "
UPDATE {$tableName} SET `value` = CONCAT(`value`, '-', row_id) 
WHERE attribute_id = '{$attributeId}' AND store_id = '{$storeId}' AND row_id <> $masterEntityId
